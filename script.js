...
function renderAdminPanel() {
    ...
    products.forEach(product => {
        const categoryIcon = product.category === "seafood" ? "🦐" : (product.category === "fish" ? "🐟" : "🥩");
        const extraImagesStr = product.extraImages ? product.extraImages.join(',') : '';
        
        const div = document.createElement("div");
        div.className = "admin-product-item";
        div.innerHTML = `
            <h3>${categoryIcon} ${escapeHtml(product.name)}</h3>
            <label>Название:</label>
            <input type="text" class="admin-name" value="${escapeHtml(product.name)}" data-id="${product.id}">
            <label>Описание:</label>
            <textarea class="admin-desc" data-id="${product.id}">${escapeHtml(product.desc)}</textarea>
            <label>Цена (₽):</label>
            <input type="number" class="admin-price" value="${product.price}" data-id="${product.id}">
            <label>Единица (кг/шт):</label>
            <input type="text" class="admin-unit" value="${product.unit}" data-id="${product.id}">
            <label>Основное фото (эмодзи):</label>
            <input type="text" class="admin-main-image" value="${escapeHtml(product.mainImage)}" data-id="${product.id}">
            <label>Дополнительные фото (3 штуки, через запятую):</label>
            <input type="text" class="admin-extra-images" value="${extraImagesStr}" data-id="${product.id}" placeholder="🦐,🍤,🍋">
            <label>Категория:</label>
            <select class="admin-category" data-id="${product.id}">
                <option value="seafood" ${product.category === "seafood" ? "selected" : ""}>🦐 Морепродукты</option>
                <option value="fish" ${product.category === "fish" ? "selected" : ""}>🐟 Рыба</option>
                <option value="meat" ${product.category === "meat" ? "selected" : ""}>🥩 Мясо</option>
            </select>
            <div class="admin-actions">
                <button class="save-product-btn" data-id="${product.id}">💾 Сохранить</button>
                <button class="delete-product-btn" data-id="${product.id}">🗑️ Удалить</button>
            </div>
        `;
    ...
    document.querySelectorAll(".save-product-btn").forEach(btn => {
        btn.addEventListener("click", () => {
            ...
            if (product) {
                product.name = document.querySelector(`.admin-name[data-id="${id}"]`).value;
                product.desc = document.querySelector(`.admin-desc[data-id="${id}"]`).value;
                product.price = parseInt(document.querySelector(`.admin-price[data-id="${id}"]`).value);
                product.unit = document.querySelector(`.admin-unit[data-id="${id}"]`).value;
                product.mainImage = document.querySelector(`.admin-main-image[data-id="${id}"]`).value;
                
                const extraImagesStr = document.querySelector(`.admin-extra-images[data-id="${id}"]`).value;
                product.extraImages = extraImagesStr.split(',').map(s => s.trim()).filter(s => s);
                
                product.category = document.querySelector(`.admin-category[data-id="${id}"]`).value;
                saveProducts();
                renderCatalog();
                renderAdminPanel();
                showMessage(`✅ Товар "${product.name}" обновлен!`);
            }
        });
    });
...
function addNewProduct() {
    ...
    renderCatalog();
    renderAdminPanel();
    showMessage("✨ Добавлен новый товар! Отредактируйте его в админ-панели");
}
